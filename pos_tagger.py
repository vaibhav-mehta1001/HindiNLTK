...
from HMMTrigram import trigramHMMViterbiAlgorithm, calculateTransitionProbabilities, calculateEmissionProbabilities
import pickle
    ...
    for Lambda in Lambdas:
        for offset in Offsets:
            for weight in Weights:
                ...
                emissionProbabilityDict = dict(calculateEmissionProbabilities(trainFile, Lambda, offset))
                print(emissionProbabilityDict)
                pickle.dump(dict(emissionProbabilityDict), open("emmission.p", "wb" ))
                # Opening the input training file
                trainFile = codecs.open("trainDataHindi.txt", mode = "r", encoding = "utf-8")
                ...
                pickle.dump(dict(transitionProbabilityDict), open("transition.p", "wb" ))
                print(allTags)
                pickle.dump(allTags, open("allTags.p", "wb"))

                # Opening the input test file
                # allTags = None
                # with open('allTags.p', 'rb') as pickle_file:
                #     allTags = pickle.load(pickle_file)
                #
                # # allTags = pickle.load("allTags.p")
                # with open('emmission.p', 'rb') as pickle_file:
                #     emissionProbabilityDict = pickle.load(pickle_file)
                #
                testFile = codecs.open("testDataHindi.txt", mode="r", encoding="utf-8")
                #
                # with open('transition.p', 'rb') as pickle_file:
                #     transitionProbabilityDict = pickle.load(pickle_file)
                # Calling the test function to get the accuracy of algorithm on the test data file
                print(transitionProbabilityDict)
                print(emissionProbabilityDict)
                print(allTags)
                accuracy = testTrigramHMMViterbiAlgorithm(testFile, allTags, emissionProbabilityDict,
                                                          transitionProbabilityDict)

                # Printing the accuracy result
                print("============================")
                print("ACCURACY : " + str(accuracy) + " %" + " " + "Offset : " + str(offset) + " " + "Lambda : " + str(
    ...
    for line in testFile.readlines():
        ...
        if sentence != []:
            ...
            try:
                ...
                predictedWordsAndTags = trigramHMMViterbiAlgorithm(sentence, allTags, emissionProbabilityDict,
                                                                   transitionProbabilityDict)
                for w in predictedWordsAndTags:
                    print(w)
